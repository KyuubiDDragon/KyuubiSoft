  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <div class="bg-white border-b border-gray-200 shadow-sm">
      <div class="max-w-4xl mx-auto px-6 py-4 flex items-center gap-3">
        ...
        <h1 class="text-lg font-bold text-gray-900">Vertragsdokument</h1>
      </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-6 py-8">
      ...
      <div v-else-if="state === 'password'" class="max-w-md mx-auto py-20">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <h2 class="text-xl font-bold text-gray-900 mb-2">Passwort erforderlich</h2>
          <p class="text-sm text-gray-500 mb-6">Dieses Dokument ist passwortgeschützt. Bitte geben Sie das Passwort ein.</p>
          ...
          <form @submit.prevent="submitPassword" class="space-y-4">
            <div>
              <input
                ...
                class="w-full px-4 py-3 rounded-xl border border-gray-300 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors"
                autofocus
              />
      ...
      <div v-else-if="state === 'error'" class="max-w-md mx-auto py-20">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <h2 class="text-xl font-bold text-gray-900 mb-2">Zugriff nicht möglich</h2>
      ...
      <template v-else-if="state === 'view' && contract">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-8">
          ...
          <!-- Parties -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 border-b border-gray-100">
            <div>
              <div class="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-2">Auftragnehmer</div>
              <div class="text-base font-semibold text-gray-900">{{ contract.party_a_name }}</div>
              <div v-if="contract.party_a_company" class="text-sm text-gray-600">{{ contract.party_a_company }}</div>
              <div v-if="contract.party_a_address" class="text-sm text-gray-500 mt-1 whitespace-pre-line">{{ contract.party_a_address }}</div>
              <div v-if="contract.party_a_email" class="text-sm text-gray-500">{{ contract.party_a_email }}</div>
            ...
            <div>
              ...
              <div class="text-base font-semibold text-gray-900">{{ contract.party_b_name }}</div>
              <div v-if="contract.party_b_company" class="text-sm text-gray-600">{{ contract.party_b_company }}</div>
              <div v-if="contract.party_b_address" class="text-sm text-gray-500 mt-1 whitespace-pre-line">{{ contract.party_b_address }}</div>
              <div v-if="contract.party_b_email" class="text-sm text-gray-500">{{ contract.party_b_email }}</div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-6 p-8 border-b border-gray-100">
            <div>
              ...
              <div class="text-sm font-medium text-gray-900 mt-1">{{ formatDate(contract.start_date) }}</div>
            </div>
            <div>
              <div class="text-xs text-gray-400 uppercase font-semibold">Vertragsende</div>
              <div class="text-sm font-medium text-gray-900 mt-1">{{ contract.end_date ? formatDate(contract.end_date) : 'Unbefristet' }}</div>
            </div>
            <div>
              <div class="text-xs text-gray-400 uppercase font-semibold">Zahlungsplan</div>
              <div class="text-sm font-medium text-gray-900 mt-1">{{ contract.payment_schedule || '-' }}</div>
            </div>
            <div>
              <div class="text-xs text-gray-400 uppercase font-semibold">Kündigungsfrist</div>
              <div class="text-sm font-medium text-gray-900 mt-1">{{ contract.notice_period_days || 30 }} Tage</div>
            </div>
          </div>

          <!-- Clauses -->
          <div v-if="contract.clauses_html" class="p-8 border-b border-gray-100">
            <div class="text-xs text-gray-400 uppercase font-semibold mb-3">Vertragsklauseln</div>
            <div class="prose prose-sm max-w-none text-gray-700" v-html="contract.clauses_html"></div>
          </div>

          <!-- Notes -->
          <div v-if="contract.notes" class="p-8">
            <div class="text-xs text-gray-400 uppercase font-semibold mb-2">Anmerkungen</div>
            <div class="text-sm text-gray-700 whitespace-pre-line">{{ contract.notes }}</div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          <div class="px-8 py-5 border-b border-gray-100 bg-gray-50">
            <h3 class="text-lg font-bold text-gray-900">Vertrag unterschreiben</h3>
            <p class="text-sm text-gray-500 mt-1">Zeichnen Sie Ihre Unterschrift in das Feld unten. Sie können auch den Touchscreen verwenden.</p>
          </div>

          <div class="p-8 space-y-4">
            <!-- Signer Name -->
            <div>
              <label class="text-sm text-gray-600 font-medium block mb-1.5">Ihr Name</label>
              <input
                v-model="signerName"
                type="text"
                placeholder="Vor- und Nachname..."
                class="w-full px-4 py-2.5 rounded-xl border border-gray-300 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors"
              />
            </div>

            <!-- Signature Canvas -->
            <div>
              <label class="text-sm text-gray-600 font-medium block mb-1.5">Unterschrift</label>
              <div class="bg-white rounded-xl overflow-hidden border-2 border-gray-200 border-dashed">
                <canvas
                  ref="canvasRef"
              ...
              <div class="flex items-center justify-between mt-2">
                <button
                  @click="clearCanvas"
                  class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors"
                >
            ...
            <!-- Error -->
            <div v-if="errorMessage" class="text-sm text-red-600 bg-red-50 rounded-lg p-3 border border-red-200">{{ errorMessage }}</div>
      ...
      <div v-else-if="state === 'signed'" class="max-w-lg mx-auto py-20">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <h2 class="text-2xl font-bold text-gray-900 mb-2">Vertrag unterschrieben</h2>
          <p class="text-sm text-gray-500 mb-4">
            Vielen Dank! Ihre Unterschrift wurde erfolgreich gespeichert.
          </p>
          <div v-if="contract" class="text-sm text-gray-600 bg-gray-50 rounded-xl p-4 text-left space-y-1">
